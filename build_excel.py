...
# ── Colors from Kennedia template ──
C_DARK_GREEN  = '375623'   # dark text
C_MED_GREEN   = '548235'   # header fill (accent6 tinted dark)
C_ACCENT      = '70AD47'   # accent6 pure green
C_LIGHT_GREEN = 'E2EFDA'   # light fill
C_ALT_ROW     = 'EBF5E1'   # alternating row
C_BORDER      = 'A9D18E'   # border color
C_NAME_BG     = 'A9D18E'   # name/unit label bg
C_WHITE       = 'FFFFFF'
C_BLACK       = '000000'
...
def font(bold=False, size=11, color=C_BLACK, name='Calibri'):
...
col_widths = {'A': 8.6, 'B': 26.8, 'C': 38.9, 'D': 8.5, 'E': 15.2, 'F': 38.9, 'G': 36.5}
...
ws['A2'] = 'Name'
ws['A2'].font = font(bold=True, size=12, color=C_DARK_GREEN)
ws['A2'].fill = solid(C_NAME_BG)
ws['A2'].alignment = align('right', 'center')
ws['A2'].border = border_all(C_BORDER)

ws['B2'] = staff_name
ws['B2'].font = font(bold=True, size=11, color=C_DARK_GREEN)
ws['B2'].fill = solid(C_LIGHT_GREEN)
ws['B2'].alignment = align('left', 'center')
ws['B2'].border = border_all(C_BORDER)

# C2:G3 merged — title area
ws.merge_cells('C2:G3')
ws['C2'] = '                   Daily Deliverables Tracker'
ws['C2'].font = font(bold=True, size=19, color=C_WHITE)
ws['C2'].fill = solid(C_MED_GREEN)
ws['C2'].alignment = align('left', 'center')
ws['C2'].border = border_medium(C_MED_GREEN)

# ── Row 3: Unit row ──
ws.row_dimensions[3].height = 23.5
ws['A3'] = 'Unit'
ws['A3'].font = font(bold=True, size=12, color=C_DARK_GREEN)
ws['A3'].fill = solid(C_NAME_BG)
ws['A3'].alignment = align('right', 'center')
ws['A3'].border = border_all(C_BORDER)

ws['B3'] = staff_unit
ws['B3'].font = font(bold=True, size=11, color=C_DARK_GREEN)
ws['B3'].fill = solid(C_LIGHT_GREEN)
ws['B3'].alignment = align('left', 'center')
ws['B3'].border = border_all(C_BORDER)

# ── Row 4: Date row (bonus - not in original) ──
ws.row_dimensions[4].height = 22
ws['A4'] = 'Date'
ws['A4'].font = font(bold=True, size=11, color=C_DARK_GREEN)
ws['A4'].fill = solid(C_NAME_BG)
ws['A4'].alignment = align('right', 'center')
ws['A4'].border = border_all(C_BORDER)
ws['B4'] = date_str
ws['B4'].font = font(bold=False, size=11, color=C_DARK_GREEN)
ws['B4'].fill = solid(C_LIGHT_GREEN)
ws['B4'].alignment = align('left', 'center')
ws['B4'].border = border_all(C_BORDER)
# Fill C4:G4 with dark green to continue title block color
for col in ['C','D','E','F','G']:
    ws[f'{col}4'].fill = solid(C_MED_GREEN)
    ws[f'{col}4'].border = border_medium(C_MED_GREEN)

# ── Row 5: Column headers ──
ws.row_dimensions[5].height = 31
headers = ['S/N','Clients/Focus Area','Proposed Task/Deliverable for the Day',
           'Priority (H/M/L)','Expected Time of Completion',
           'Actual Deliverables for Today','Achievement/Result for the Day']
for i, h in enumerate(headers):
    cell = ws.cell(row=5, column=i+1)
    cell.value = h
    cell.font = font(bold=True, size=12 if i != 1 else 13, color=C_WHITE)
    cell.fill = solid(C_MED_GREEN)
    cell.alignment = align('center' if i == 0 else 'left', 'center', wrap=True)
    cell.border = border_all(C_MED_GREEN)

# ── Task rows ──
for idx, task in enumerate(tasks):
    row = idx + 6
    row_h = max(40, 15 * max(1, len(str(task.get('proposed','') or '')) // 30))
    ws.row_dimensions[row].height = row_h
    
    is_alt = (idx % 2 == 1)
    row_fill = solid(C_ALT_ROW if is_alt else 'FFFFFF')
    ...
    for ci, val in enumerate(vals):
        ...
        cell.value = val
        cell.font = font(bold=False, size=11, color=C_DARK_GREEN)
        cell.fill = row_fill
        cell.alignment = align('center' if ci == 0 else 'left', 'center', wrap=True)
        cell.border = border_all(C_BORDER)

# Instructions text is embedded in the title area
...
try:
    ...
    print(f'Logo embed warning: {e}', file=sys.stderr)
